...
interface Assignment { id: number; certificate: Certificate; person: Person; createdAt: string }
...
export default function AsignacionesPage() {
    ...
    const [form, setForm] = useState({ certificateId: "", identification: "" })
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const res = await fetch("/api/asignaciones", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) })
        if (res.ok) {
            toast.success("Certificado asignado")
            setForm({ certificateId: "", identification: "" })
            fetchData()
            setShowModal(false)
        } else {
    ...
    const viewCert = (a: Assignment) => { setPreviewHtml(a.certificate.templateHtml.replace(/\{\{NOMBRE_COMPLETO\}\}/g, `${a.person.firstName} ${a.person.lastName}`).replace(/\{\{IDENTIFICACION\}\}/g, a.person.identification).replace(/\{\{TIPO_PARTICIPACION\}\}/g, a.certificate.participationType).replace(/\{\{NOMBRE_EVENTO\}\}/g, a.certificate.event?.name || "").replace(/\{\{FECHA_EXPEDICION\}\}/g, new Date(a.certificate.issueDate).toLocaleDateString("es-CO"))); setShowPreview(true) }
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                ...
                <button onClick={() => { setForm({ certificateId: certificates[0]?.id?.toString() || "", identification: "" }); setError(""); setShowModal(true) }} className="inline-flex items-center gap-2 rounded-lg bg-brand-500 px-5 py-2.5 text-sm font-medium text-white shadow-theme-xs hover:bg-brand-600 transition-colors">+ Asignar</button>
            </div>
                        ...
                        {assignments.map((a) => (
                            <tr key={a.id} className="hover:bg-gray-50 transition-colors">
                                ...
                                <td className="px-6 py-4 text-right space-x-2">
                                    ...
                                    <button onClick={() => handleDelete(a.id)} className="inline-flex rounded-lg border border-error-100 bg-error-50 px-3 py-1.5 text-xs font-medium text-error-600 hover:bg-error-100 transition-colors">Eliminar</button>
                                </td>
                            </tr>
                        ))}
                        {assignments.length === 0 && <tr><td colSpan={5} className="px-6 py-12 text-center text-gray-400 text-sm">No hay asignaciones</td></tr>}
                    </tbody>
                </table></div></div>
            )}
            {showModal && (
                <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-gray-900/50 p-4">
                    <div className="w-full max-w-md rounded-2xl border border-gray-200 bg-white p-6 shadow-theme-lg">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <div><label className="block text-sm font-medium text-gray-700 mb-2">Certificado</label><select value={form.certificateId} onChange={e => setForm({ ...form, certificateId: e.target.value })} required className={ic}><option value="">Seleccione</option>{certificates.map(c => <option key={c.id} value={c.id}>{c.event?.name} - {c.participationType}</option>)}</select></div>
